...
void Drawer::run() {
    ...
    SDL_Texture* videoTexture = SDL_CreateTexture(main.getRenderer(),
            SDL_PIXELFORMAT_RGB24, SDL_TEXTUREACCESS_TARGET, main.getWidth(), main.getHeight());
    assert(videoTexture != NULL);
    long fixed_time = 1000 / 60; // segundos / frames

    while (running){
        std::vector<char> currentFrame(main.getWidth() * main.getHeight() * 3);
        ...
        int res = SDL_RenderReadPixels(main.getRenderer(), NULL,
                SDL_PIXELFORMAT_RGB24, currentFrame.data(), main.getWidth() * 3);
        if (res) {
